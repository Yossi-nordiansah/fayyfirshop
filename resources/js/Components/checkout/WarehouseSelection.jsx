...
export default function WarehouseSelection({
                ...
                <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
                    <div className="flex-1">
                        <div className="flex items-center gap-2">
                            <span className="text-xs font-bold uppercase tracking-wider text-slate-400">{branch.code} Warehouse</span>
                    ...
                    <div className="flex items-center gap-1 text-[10px] font-bold self-start md:self-auto">
                        {hasAllStock ? (
                            <span className="text-emerald-700 bg-emerald-50 px-2 py-1 border border-emerald-200 rounded-full">{t("checkout.warehouse.stock_ok", "Stok Cukup")}</span>
                        ) : (
                            <span className="text-amber-700 bg-amber-50 px-2 py-1 border border-amber-200 rounded-full">{t("checkout.warehouse.stock_partial", "Sebagian Kosong")}</span>
                        )}
                    </div>
                </div>
            </section>
    ...
    return (
        <section className="p-6 bg-white border border-slate-100 shadow-sm rounded-3xl">
            <h2 className="text-base font-extrabold text-slate-900 pb-4 mb-4 border-b border-slate-100 flex items-center gap-2">
                <Store className="text-amber-500" size={18} />
                {t("checkout.warehouse_section", "Gudang Pengirim (Multi-Warehouse)")}
            </h2>
            ...
            <div className="grid gap-3 md:grid-cols-3">
                {filteredBranches.map(branch => {
                    ...
                    // Count stock status of cart items in this branch
                    const branchStocks = stocksData[branch.id] || [];
                    const hasAllStock = cartItems.every(item => {
                        const stockItem = branchStocks.find(s => s.id === item.id && s.variantId === item.variantId);
                        return stockItem && stockItem.stock >= item.quantity;
                    ...
                    return (
                        ...
                        >
                            ...
                            <div className="mt-4 flex items-center gap-1 text-[10px] font-bold">
                                {hasAllStock ? (
                                    <span className="text-emerald-700 bg-emerald-50 px-2 py-0.5 border border-emerald-200 rounded-full">{t("checkout.warehouse.stock_ok", "Stok Cukup")}</span>
                                ) : (
                                    <span className="text-amber-700 bg-amber-50 px-2 py-0.5 border border-amber-200 rounded-full">{t("checkout.warehouse.stock_partial", "Sebagian Kosong")}</span>
                                )}
                            </div>
                        </button>
                    );
